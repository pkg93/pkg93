...
`,localStorage[".pkg93/config.json"]===void 0&&(localStorage[".pkg93/config.json"]="{\"repos\": [\"http://codinggamerhd.com/main-repo\"], \"installed\": [], \"pkglist\": []}"),localStorage[".pkg93/packages/"]===void 0&&(localStorage[".pkg93/packages/"]=""),localStorage[".pkg93/README.txt"]="WARNING!\nThis folder contains important data about pkg93. Do not edit anything in here unless you want pkg93 to not work!\n\n~1024x2";var d=pkg93.getConfig();if(0===b.length)$log(help);else if("pull"==b[0])pkg93.pull();else if("get"==b[0])2>b.length?$log("<b><span style='color:#f00'>ERR</span></b>  No package specified."):a.includes(b[1])?$log("<b><span style='color:#f00'>ERR</span></b>  You're trying to modify a pre-installed Windows93 app.\n      <b>Don't do that!</b>"):pkg93.get(b[1]);else if("rm"==b[0])2>b.length?$log("<b><span style='color:#f00'>ERR</span></b>  No package specified."):a.includes(b[1])?$log("<b><span style='color:#f00'>ERR</span></b>  You're trying to modify a pre-installed Windows93 app.\n      <b>Don't do that!</b>"):pkg93.rm(b[1]);else if("add-repo"==b[0]){if(2>b.length)$log("<b><span style='color:#f00'>ERR</span></b>  No repository specified.");else try{d.repos.push(b[1]),localStorage[".pkg93/config.json"]=JSON.stringify(d),$log("<b><span style='color:#0f0'>OK</span></b>   Done!\n     Run \"pkg93 pull\" to update the package listing.")}catch(f){$log("<b><span style='color:#f00'>ERR</span></b>  "+f.message)}}else if("rm-repo"==b[0]){if(2>b.length)$log("<b><span style='color:#f00'>ERR</span></b>  No repository specified.");else try{d.repos.splice(parseInt(b[1]),1),localStorage[".pkg93/config.json"]=JSON.stringify(d),$log("<b><span style='color:#0f0'>OK</span></b>   Done!\n     Run \"pkg93 pull\" to update the package listing.")}catch(f){$log("<b><span style='color:#f00'>ERR</span></b>  "+f.message)}}else if("ls"==b[0])"pkgs"==b[1]?$log(d.pkglist.join("\n")):"installed"==b[1]?$log(d.installed.join("\n")):"repos"==b[1]?(lerepos="",d.repos.forEach(function(f,g){lerepos+="["+g+"] "+f+"<br>"}),$log(lerepos)):$log("<b><span style='color:#f00'>ERR</span></b>  You must select either pkgs, installed, or repos.");else if(!("info"==b[0]))"help"==b[0]?$log(help):"wtf"==b[0]?(new Audio("/c/sys/sounds/QUACK.ogg").play(),wtf=["mudkipz","pkg93","memes","linux","javascript","git","cpu","windows93","discord","kirb","apt93","delays","trash","kernel panic","bash","package manager","recusion"],$log("<b><span style='color:#0f0'>WTF?</span></b> "+wtf[Math.floor(Math.random()*wtf.length)]+" + "+wtf[Math.floor(Math.random()*wtf.length)]+" = "+wtf[Math.floor(Math.random()*wtf.length)])):$log("<b><span style='color:#f00'>ERR</span></b>  Invalid command. Type \"pkg93\" without any arguments for help.");else if(2<b.length)$log("<b><span style='color:#f00'>ERR</span></b>  No package specified.");else{var e=JSON.parse(pkg93.pkgInfo(b[1]));e?(depends=e.dependencies?e.dependencies.join(" , "):"<i><span style='color:#444'>None!</span></i>",console.log(e),$log(`<b><u>${e.name}</u></b>
Description: ${e.description}
Dependencies: ${depends}`)):$log("<b><span style='color:#f00'>ERR</span></b>  Either the package doesn't exist, or an error occoured.")}},icon:"//cdn.rawgit.com/1024x2/pkg93/70039c02/pkg.png",terminal:!0,hascli:!0,categories:"Network;Utility;Settings;PackageManager"};
